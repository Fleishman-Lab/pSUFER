...
rule rosetta_relax:
  ...
  params:
    dir='temp/{D}'
  run:
    command = f'{config["rosetta_exec"]} -parser:protocol xmls/refine.xml -s {input.pdb}  @flags/flags_refinement -out:prefix {params.dir}/{wildcards.num}? -out:file:scorefile {params.dir}/{wildcards.A}.sc '
    print(command)
    os.system(command)
